...
def label_propagation_community_detection(G):
    ...
    # Continue updating labels until convergence
    while True:
        changes = False
        random.shuffle(nodes)  # To ensure random order in each iteration
        ...
        # If no changes were made, the algorithm has converged
        if not changes:
            break
...
def use_knee(G):
    ...
    optimal_eps = kneedle_eps.knee
    optimal_mu = kneedle_mu.knee 
    
    return optimal_eps, optimal_mu

def main():
    G=nx.Graph()
    # filename = "data/TC1-1/1-1.dat"
    for name in ["tc11.dat", "tc12.dat", "tc13.dat", "tc14.dat", "tc15.dat"]:
        # filename = "data/TC2/tc11.dat"
        filename = f"data/TC2/{name}"
        ...
        learning_rate_mu = 0.1  # Learning rate
        ...
        eps, mu, ari_list, nmi_list = gradient_descent_2d(G, loss, initial_eps, initial_mu, learning_rate_eps, learning_rate_mu, n_iterations)
        print(filename)
        print(f"Time taken: {time.time() - start} seconds")
    # plot_metrics(ari_list, nmi_list)
    # clusters = scan(G, initial_eps, initial_mu)
    # clusters = label_propagation_community_detection(G)
    # ground_truth_labels = list(ground_truth.values())
    # computed_labels = convert_to_labels(list(G.nodes()), clusters)
    # ari, nmi = compare_clusters(ground_truth_labels, computed_labels)
    # print(f"LP: Adjusted Rand Index: {ari}")
    # print(f"LP: Normalized Mutual Information: {nmi}")
